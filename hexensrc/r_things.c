...
void R_DrawFuzzColumnLow (void); //rww begin rww end
...
//		colfunc = fuzzcolfunc;	// NULL colormap = shadow draw
	...
	if(vis->mobjflags&(MF_SHADOW|MF_ALTSHADOW))
	{
		...
		{
			colfunc = transfuzzcolfunc; //rww begin rww end
		...
		{
			colfunc = altfuzzcolfunc; //rww begin rww end
		}
	}
	else if(vis->mobjflags&MF_TRANSLATION)
	{
		// Draw using translated column function
		colfunc = transcolfunc;//R_DrawTranslatedColumn; //rww begin rww end
		dc_translation = translationtables-256
			+vis->class*((MAXPLAYERS-1)*256)+
			((vis->mobjflags&MF_TRANSLATION)>>(MF_TRANSSHIFT-8));
	}
	...
	if(vis->psprite)
	{
		dc_texturemid += FixedMul(((centery-HexenDivInt(viewheight,2))<<FRACBITS), //rww begin rww end
			dc_iscale); //rww begin rww end (use proper detailshifted scale instead of vis->xiscale)
		sprtopscreen += (HexenDivInt(viewheight,2)-centery)<<FRACBITS; //rww begin rww end
	}
...
//
	...
	if(viewangleoffset)
	{
		tempangle = ((HexenDivInt(centerxfrac,1024))*(viewangleoffset>>ANGLETOFINESHIFT)); //rww begin rww end
	}
	else
	{
		tempangle = 0;
