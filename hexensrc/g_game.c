...
void TouchScreenSample(void)
{
	...
	if(touch.px > 0 && touch.py > 0) {
		char tempstringyay[50];
		sprintf(tempstringyay, "Touch (%d,%d)", touch.px, touch.py);
	...
	*/
	...
	if(touch.py > 160) {
		if(!inventory && touch.px >= 140 && touch.px <= 160 && touch.py < 180) {
			inventoryTics = 5*35;
			inventory = true;
		} else if(!inventory && touch.py < 180 && touch.px >= 120 && touch.px <= 140 && players[consoleplayer].inventory[inv_ptr - curpos].type != arti_none) {
			// use the acive item
			gamekeydown[key_useartifact] = true;
			RegisterDSKey(key_useartifact, 0);
			RegisterDSKey(key_useartifact, 1);
		} else if(inventory && touch.py < 180 && inventoryTics < 5*35 - 5) {
			// find which slot was touched
			int oldcurpos = curpos;
			if(touch.px >= 44 && touch.px < 68 && players[consoleplayer].inventory[0 + inv_ptr - curpos].type != arti_none) {
				curpos = 0;
				inventory = false;
			} else if(touch.px >= 68 && touch.px < 92 && players[consoleplayer].inventory[1 + inv_ptr - curpos].type != arti_none) {
				curpos = 1;
				inventory = false;
			} else if(touch.px >= 92 && touch.px < 116 && players[consoleplayer].inventory[2 + inv_ptr - curpos].type != arti_none) {
				curpos = 2;
				inventory = false;
			} else if(touch.px >= 118 && touch.px < 142 && players[consoleplayer].inventory[3 + inv_ptr - curpos].type != arti_none) {
				curpos = 3;
				inventory = false;
			} else if(touch.px >= 144 && touch.px < 168 && players[consoleplayer].inventory[4 + inv_ptr - curpos].type != arti_none) {
				curpos = 4;
				inventory = false;
			} else if(touch.px >= 168 && touch.px < 192 && players[consoleplayer].inventory[5 + inv_ptr - curpos].type != arti_none) {
				curpos = 5;
				inventory = false;
			} else if(touch.px >= 194 && touch.px < 218 && players[consoleplayer].inventory[6 + inv_ptr - curpos].type != arti_none) {
				curpos = 6;
				inventory = false;
			}

			inv_ptr += curpos - oldcurpos;
			if(inv_ptr >= players[consoleplayer].inventorySlotNum)
				inv_ptr--;
			if(inv_ptr < 0)
				inv_ptr = 0;

			// make sure we redraw the newly selected item on the main statusbar
			if(!inventory)
				players[consoleplayer].readyArtifact = players[consoleplayer].inventory[inv_ptr].type;

			// scroll inventory bar if necessary
			if(curpos == 0 && inv_ptr > 0) {
				inv_ptr--;
				curpos++;
			} else if(curpos == 6 && inv_ptr < players[consoleplayer].inventorySlotNum - 1) {
				inv_ptr++;
				curpos--;
			}
		}
...
void G_DoPlayDemo (void)
{
	...
	precache = true;               // don't spend a lot of time in loadlevel
	G_InitNew (skill, episode, map);
	precache = false;
	usergame = false;
	demoplayback = true;
