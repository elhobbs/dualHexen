...
void MN_Drawer(void)
{
	...
	if(MenuActive == false)
	{
		...
		{
			/*
			if(cartSetMenuMode(DEVICE_TYPE_AUTO))  {     //Prepare the Slot2 device for reset
					passmeloopEnter();      //Enter PassMe loop.
					}
			*/
			MN_DrTextA(QuitEndMsg[typeofask-1], 125- //Sektor - used to be 160 but the text went off the screen
...
boolean MN_Responder(event_t *event)
{
	...
	if(askforquit)
	{
		...
		{
			//case 'm': //Sektor
			case KEY_ENTER: //Sektor 2011/02/12
				if(askforquit)
				{
					switch(typeofask)
					{
