...
#ifdef _HEXENDS
#define GET_BYTE (*SavePtr.l++)
#define GET_WORD (*SavePtr.l++)
#define GET_LONG (*SavePtr.l++)
#define ALIGN_FOUR(x) (x % 4 == 0 ? 0 : 4 - (x % 4))
#else
#define GET_BYTE (*SavePtr.b++)
#define GET_WORD (*SavePtr.w++)
#define GET_LONG (*SavePtr.l++)
#define ALIGN_FOUR(x) 0
#endif
...
typedef struct
{
	...
	void (*restoreFunc)();
	size_t size;
} STRUCTSPACKED thinkInfo_t;

typedef struct
{
	thinker_t thinker;
	sector_t *sector;
} STRUCTSPACKED ssthinker_t;
...
//#if _HEXENDS_GBFS //rww begin FIXME
	if(!deathmatch)
	{
...
static void UnarchivePlayers(void)
{
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		...
		SavePtr.b += sizeof(player_t) + ALIGN_FOUR(sizeof(player_t));
...
static void UnarchiveMobjs(void)
{
	...
	for(i = 0; i < MobjCount; i++)
	{
		...
		SavePtr.b += sizeof(mobj_t) + ALIGN_FOUR(sizeof(mobj_t));
	...
	while((tClass = GET_BYTE) != TC_NULL)
	{
		for(info = ThinkerInfo; info->tClass != TC_NULL; info++)
		{
			if(tClass == info->tClass)
			{
				...
				SavePtr.b += info->size + ALIGN_FOUR(info->size);
...
static void UnarchiveScripts(void)
{
	...
	SavePtr.b += sizeof(MapVars) + ALIGN_FOUR(sizeof(MapVars));
}
...
static void AssertSegment(gameArchiveSegment_t segType)
{
	...
	temp = GET_LONG;
	if(temp != segType) {
		I_Error("Corrupt save game: Segment [%d] failed alignment check:  got %d",
			segType, temp);
	}
}
...
static void StreamOutBuffer(void *buffer, int size)
{
	...
	char zero = 0;
	switch(size % 4) {
		case 1: fwrite(&zero, 1, 1, SavingFP);
		case 2: fwrite(&zero, 1, 1, SavingFP);
		case 3: fwrite(&zero, 1, 1, SavingFP);
	}
#endif
}
...
#ifdef _HEXENDS
	int i = val;
	fwrite(&i, sizeof(int), 1, SavingFP);
#else
...
static void StreamOutLong(unsigned int val)
{
	fwrite(&val, sizeof(unsigned int), 1, SavingFP);
}
