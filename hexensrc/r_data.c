...
byte *R_GetColumn (int tex, int col)
{
	...
	if (lump > 0)
		return (byte *)W_CacheLumpNumTexture(textures[tex], lump,PU_CACHE, tex)+ofs;
...
//rww begin
int R_OtherTextureUsingLump(texture_t *tex, int lump)
{
	int i, j;
	for (i=0 ; i<numtextures ; i++)
	{
		...
		{
			if (texture->patches[j].patch == lump)
			{
				return 1;
			}
		}
