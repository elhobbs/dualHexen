...
static int ReadFile(char const *name, byte **buffer, int mallocType)
{
	...
	{ // Use c library memory allocation
		buf = malloc(length); //rww malloc->ds_malloc
		if(buf == NULL)
		{
...
default_t defaults[] =
{
	{ "mouse_sensitivity", &mouseSensitivity, 3 },//5 },
...
//rww end
		...
		#define DEFAULT_SAVEPATH		"/data/hexen/"
	#else
...
{
#ifndef _HEXENDS_GBFS //rww begin FIXME - use sram
	int     i,v;
	FILE    *f;
...
void M_LoadDefaults(char *fileName)
{
	...
	{
#ifdef _HEXENDS
		sprintf(defaultfile, "/data/hexen/%s", fileName);
#else
