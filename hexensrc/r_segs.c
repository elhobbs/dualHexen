...
//
	...
	// calculate lighting
		if (maskedtexturecol[dc_x] != MAXSHORT)
		{
			...
			dc_iscale = HexenDivIntU(0xffffffffu , (unsigned)spryscale); //rww begin rww end
...
//
		...
		// calculate lighting
			...
			dc_colormap = walllights[index];
			dc_x = rw_x;
			dc_iscale = HexenDivIntU(0xffffffffu , (unsigned)rw_scale); //rww begin rww end
		}
...
//
	...
	if (stop > start )
	{
		...
		ds_p->scalestep = rw_scalestep =
			HexenDivInt((ds_p->scale2 - rw_scale) , (stop-start)); //rww begin rww end
	}
	else
	{
	//
